...
class AutoClicker:
    ...
    def click_at(x, y):
        try:
            # Генерация случайных смещений в диапазоне от -3 до 3 пикселей
    ...
    def click_on_targets(self, hsv, monitor, sct):
        for target_hsv in self.target_hsvs:
            ...
            # Генерация случайного процента нажатий в диапазоне 10-15% от заданного
            random_percentage = random.uniform(self.target_percentage * 0.80, self.target_percentage * 1.20)
            num_to_click = int(num_contours * random_percentage)

            self.logger.log(f"Отклонение процента: {random_percentage:.2f}, заданный процент: {self.target_percentage:.2f}, общее количество контуров: {num_contours}, количество кликов: {num_to_click}")

            # Убедимся, что мы не пытаемся кликнуть больше контуров, чем у нас есть
            num_to_click = min(num_to_click, num_contours)
            ...
            for contour in reversed(contours_to_click):
                ...
                cY = int(M["m01"] / M["m00"]) + monitor["top"]
                 ...
                 # Генерация случайного отклонения для координат
                random_offset_x = random.randint(-30, 30)
                random_offset_y = random.randint(-30, 30)
                final_cX = cX + random_offset_x
                final_cY = cY + random_offset_y

                # Логируем отклонение координат
                self.logger.log(f"Отклонение координат: ({random_offset_x}, {random_offset_y}), финальные координаты: ({final_cX}, {final_cY})")
                ...
                cY += 5
                self.click_at(cX, cY)
                # Предполагается, что у вас есть метод logger
                # self.logger.log(f'Нажал: {cX} {cY}')
                self.clicked_points.append((cX, cY))

                # Случайный интервал между кликами от 0 до 150 мс
                random_delay = random.uniform(0, 0.15)  # время в секундах
                time.sleep(random_delay)  # Пауза между кликами

        if self.collect_freeze:  # Теперь это правильно, находится внутри метода
...
if __name__ == "__main__":
    ...
    logger = Logger("[flfxidk]")
    logger.log("Бот - Автокликер")
